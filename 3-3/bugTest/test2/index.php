...
if (isset($_POST["signUp"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && $_POST["password"] === $_POST["password2"]) {
        ...
        $dsn = sprintf('mysql: host=%s; dbname=%s; charset=utf8', $_SESSION['host'], $_SESSION['dbname']);

        // 3. エラー処理
        try {
            $pdo = new PDO($dsn, $_SESSION['user'], $_SESSION['pass'], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
